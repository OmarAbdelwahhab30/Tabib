        ...
        <nav class="container-fluid header-in">
            <div class="row">
                ...
                <div class="col-xl-4 d-none d-xl-block">
                    ...
                    <a href="dashboard-add-doctor.php" class="btn btn-sm border-radius-30 margin-tb-15px text-white background-second-color  box-shadow float-right padding-lr-25px margin-left-30px"><i class="fas fa-plus-circle  margin-right-7px"></i>Add Doctor </a>
                ...
                <div class="row margin-tb-45px full-width">
                    <div class="col-md-4">
                        <div class="padding-15px background-white">
                            <?php
                            $adminusername = $_SESSION['ADMIN_NAME'] ;
                            $sql_statement = queryHandler::getData("admins","*","username = '$adminusername'");
                            if ($sql_statement) {
                                $result = $sql_statement->fetch(PDO::FETCH_ASSOC);
                            }
                            ?>
                            <form action="imgProcess.php?adminid=<?=$result['id']?>" method="post" enctype="multipart/form-data">
                                <a href="#" class="d-block margin-bottom-10px"><img src="imgs/<?=isset($result['img_path'])? $result['img_path']:""?>" alt=""></a>
                    ...
                    <div class="col-md-8">
                        <form action="updateAdminProfile.php?adminid=<?=$result['id']?>" method="post">
                            <div class="row">
                                <div class="col-md-6 margin-bottom-20px">
                                    <label><i class="far fa-user margin-right-10px"></i>Doc. UserName</label>
                                    <input type="text" name="adminUername" value="<?=$result['username']?>" class="form-control form-control-sm" placeholder="name" required>
                                </div>
                                <div class="col-md-6 margin-bottom-20px">
                                    <label><i class="fas fa-lock margin-right-10px"></i> Password</label>
                                    <input type="password" name="password" value="<?=$result['password']?>" class="form-control form-control-sm" placeholder="password" required>
                                </div>
                                <div class="col-md-6 margin-bottom-20px">
                                    <label><i class="fas fa-lock margin-right-10px"></i>Confirm Password</label>
                                    <input type="password"  name="confpassword" value="<?=$result['password']?>" class="form-control form-control-sm" placeholder="Confirm password" required>
                                </div>
                                <div class="col-md-6 margin-bottom-20px">
                                    <label><i class="far fa-user margin-right-10px"></i>First Name</label>
                                    <input type="text" name="firstname" value="<?=$result['first_name']?>" class="form-control form-control-sm" placeholder="first name" required>
                                </div>
                                <div class="col-md-6 margin-bottom-20px">
                                    <label><i class="fas fa-user margin-right-10px"></i> Last Name</label>
                                    <input type="text" name="lastname" value="<?=$result['last_name']?>" class="form-control form-control-sm" placeholder="last name" required>
                                </div>
                                <div class="col-md-6 margin-bottom-20px">
                                    <label><i class="fas fa-mobile-alt margin-right-10px"></i> Address</label>
                                    <input type="text" name="address" value="<?=$result['address']?>" class="form-control form-control-sm" placeholder="any address" required>
                                </div>
                                <div class="col-md-6">
                                    <label><i class="fas fa-link margin-right-10px"></i> Phone</label>
                                    <input type="text" name="phone" value="<?=$result['phone']?>" class="form-control form-control-sm" placeholder="0123456789" required>
                                </div>
                                <div class="col-md-6">
                                    <label><i class="fas fa-info margin-right-10px"></i> Email</label>
                                    <input type="text" name="email" value="<?=$result['email']?>" class="form-control form-control-sm" placeholder="any@hotmail.com" required>
                                </div>
                            </div>
