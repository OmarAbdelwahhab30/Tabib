        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
            ...
            <ul class="navbar-nav navbar-sidenav background-main-color admin-nav" id="admin-nav">
                ...
                <li class="nav-item" data-toggle="tooltip" data-placement="right" title="Doctors">
                    <a class="nav-link active" href="dashboard-my-doctors.php">
                ...
                <li class="nav-item" data-toggle="tooltip" data-placement="right" title="Sign Out">
                    <a class="nav-link" href="pateint-logout.php" data-toggle="modal" data-target="#exampleModal">
            ...
            <div class="row margin-tb-90px margin-lr-10px sm-mrl-0px">
                ...
                <div id="page-title" class="padding-30px background-white full-width box-shadow">
                    <div class="container">
                        ...
                        <h1 class="font-weight-300">Doctors</h1>
                    </div>
                </div>
                <!-- // Page Title -->
                <div class="margin-tb-45px">
                   <?php
                    ...
                    require_once 'patient-pagination-f-doc.php';
                   
                    $doctors_boolean = queryHandler::getData('doctors','*',"1 ORDER BY id LIMIT $offset,$limit");
                    if($doctors_boolean){
                        $data_for_doctors = $doctors_boolean->fetchALL(PDO::FETCH_ASSOC);
                        if(!empty($data_for_doctors)){
                            foreach($data_for_doctors as $one_row){ ?>
                                <div class="background-white thum-hover box-shadow hvr-float full-width margin-bottom-45px" style="width: 600px;">
                                    <div class="float-lg-left margin-right-30px sm-mr-0px text-center sm-mt-35px">
                                        <img src="../Doctor/imgs/<?=isset($result['img_path'])? "{$result['img_path']}":"profile.png"?>" alt="" style="border-radius:5px; width: 250px; height:220px">
                                    </div>
                                    <div class="padding-lr-25px padding-tb-45px" >
                                        <h2>
                                            ...
                                            <span class="text-grey-2"><i class="far fa-map"></i> <?="SPEC. : ". $one_row['specialization'] ?> </span><br>
                                        </h2>
                                        ...
                                        <span class="text-grey-2"><i class="far fa-map"></i> <?= "Time of presence : ". $one_row['time_of_presense'] ?> </span><br>
                                        <span class="text-grey-2"><i class="far fa-map"></i> <?= "Stay time : ". $one_row['stay_time'] ?> </span><br>
                                        <span class="text-grey-2"><i class="far fa-map"></i> <?= "subscribtion : ". $one_row['subscribe'] ?> </span><br>
                                        <?php
                                            $exist_boolean = queryHandler::getData('subscribtion','*',"doctor_id='{$one_row['id']}' AND patient_id ='$patient_Id'");
                                            $array_exist_subiscribtion = $exist_boolean->fetch(PDO::FETCH_ASSOC);

                                            if($array_exist_subiscribtion){ ?>
                                                <a class="page-link" href="patient-add-subscribtion.php?id=<?= $one_row['id'] ?>">unvote</a>
                                           <?php } else { ?>
                                                <a class="page-link" href="patient-add-subscribtion.php?id=<?= $one_row['id'] ?>">vote</a>
                                          <?php }
                                        ?>    
                                    </div>
                            ...
                            <?php }
                                ...
                                <div class="background-white thum-hover box-shadow hvr-float full-width margin-bottom-45px" style="width: 600px;">
                                    <div class="padding-lr-25px padding-tb-45px" >
                                        <h2 class="font-weight-300" style="margin-left: 60px;color:dodgerblue">No Doctors To Show</h2>   
                                    </div>
                                </div>
                       <?php
                        }
                    ...
                    <?php
                        if(!empty($data_for_doctors)){ ?>
                            <ul class="pagination pagination-md ">
                                <li class="page-item "><a class="page-link rounded-0" href="dashboard-my-doctors.php?state=previous">Previous</a></li>
                                <?php 
                                    for($i = 1; $i <= $number_of_pages;$i++){ ?>
                                        <li class="page-item"><a class="page-link" href="dashboard-my-doctors.php?num=<?= $i ?>"><?= $i ?></a></li>    
                                <?php } ?>
                                <li class="page-item"><a class="page-link rounded-0" href="dashboard-my-doctors.php?state=next">Next</a></li>
                            </ul>
                       <?php }
                    ?>
