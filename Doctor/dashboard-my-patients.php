            ...
            <div class="row">
                ...
                <div class="col-xl-4 d-none d-xl-block">
                    ...
                    <div class="nav-item dropdown float-left">
                        ...
                        ?>
                        <a href="dashboard-my-profile.php" class="margin-top-15px d-inline-block text-grey-3 margin-right-15px"><img src="imgs/<?=isset($result['img_path'])? "{$result['img_path']}":"profile.png"?>" class="height-30px border-radius-30" alt=""><?=" ".$_SESSION['DOCTOR_USERNAME']." "?></a>
                    </div>
        ...
        <div class="container-fluid overflow-hidden">
            <div class="row margin-tb-90px margin-lr-10px sm-mrl-0px">
                ...
                <div class="margin-tb-45px full-width">
                    ...
                    $query = "SELECT patients.img_path,patients.username, appointment.appointment_id, patients.first_name , 
                    patients.last_name,patients.email,patients.gender,patients.age ,doctors.id FROM appointment JOIN patients 
                    ON patients.id = appointment.patient_id 
                    JOIN doctors ON appointment.doctor_id = doctors.id  where doctors.id ='$DOCTOR_ID'";
                    ...
                    if (!empty($result)){
                            ...
                            <div class="background-white thum-hover box-shadow hvr-float full-width margin-bottom-45px">
                                <div class="float-lg-left margin-right-30px sm-mr-0px text-center sm-mt-35px">
                                    <img style="margin: 50px 0 50px 20px; height: 100px;width:69px;" src="../patientPhotos/<?=isset($index['img_path'])? "{$index['img_path']}":"profile.png"?>" class="float-left margin-right-20px border-radius-60 margin-bottom-20px" alt="">
                                </div>
                                <div >
                                    <h2>
                                        <a href="#" class="d-inline-block text-dark text-capitalize text-medium margin-tb-15px"><?=$index['first_name']." ".$index['last_name']?></a>
                                        <a href="deleteAppointment.php?Appointment_id=<?=$index['appointment_id']?>" class="d-inline-block margin-lr-10px text-grey-2 text-up-small"><i class="far fa-window-close"></i> Delete</a>
                                    </h2>
                                    <span class="text-grey-2"><i class="far fa-map"></i> <span style="color: #0b2e13">Email</span> : <?=" ".$index['email']?></span><br>
                                    <span class="text-grey-2"><i class="far fa-map"></i> <span style="color: #0b2e13">Gender</span> : <?=" ".$index['gender']?></span><br>
                                    <span class="text-grey-2"><i class="far fa-map"></i> <span style="color: #0b2e13">Age</span> : <?=" ".$index['age']?></span><br>
                                    <span class="text-grey-2"><i class="far fa-map"></i> <span style="color: #0b2e13">username</span> : <?=" ".$index['username']?></span><br>
