                ...
                <div class="col-12">
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            ...
                            $query ="SELECT patients.username,appointment.appointment_id, patients.first_name ,appointment.cause ,appointment.day_appointment,
                        ...
                        <div class="col-lg-6">
                            <!-- -->
                            <?php
                            $query = "SELECT patients.img_path,patients.username,testimonials.content, testimonials._datetime, patients.first_name , patients.last_name FROM testimonials INNER JOIN patients ON testimonials.patient_id = patients.id LIMIT 3";
                            $sql_statement = $GLOBALS['connection']->prepare($query);
                            $sql_statement->execute();
                            $result = $sql_statement->fetchall(PDO::FETCH_ASSOC);
                            ?>
                            <div class="margin-bottom-30px">
                                    ...
                                    <ul class="commentlist padding-0px margin-0px list-unstyled text-grey-3">
                                        ...
                                        ?>
                                            <li class="border-bottom-1 border-grey-1 margin-bottom-20px">
                                                <img style=" height: 70px;width:70px;" src="../patientPhotos/<?=isset($index['img_path'])? "{$index['img_path']}":"profile.png"?>" class="float-left margin-right-20px border-radius-60 margin-bottom-20px" alt="">
                                                ...
                                                <p class="margin-top-15px text-grey-2"><?=$index['content']?></p>
                                            </div>
                                        </li>
                                        <?php
                                        }
                                        ...
                                        <div class="background-white thum-hover box-shadow hvr-float full-width margin-bottom-45px">
                                            <p class="text-grey-2" style="color: dodgerblue ;padding:22px ; font-size: 22px; ">No Data To Show</p></div>
                                        <?php } ?>
                                    </ul>
                                </div><br>
                                ...
                                <div class="padding-30px background-white border-radius-20 box-shadow">
                                    ...
                                    <hr>
                                    <?php
                                    $query = "SELECT doctors.img_path,doctors.username,tips.doctor_tip, tips._datetime, doctors.first_name , doctors.last_name FROM tips 
                                    INNER JOIN doctors ON tips.doctor_id = doctors.id ORDER BY _datetime DESC LIMIT 3";
                                    $sql_statement = $GLOBALS['connection']->prepare($query);
                                    $sql_statement->execute();
                                    $result = $sql_statement->fetchall(PDO::FETCH_ASSOC);
                                    if (!empty($result)){
                                        foreach ($result as $index){
                                            ...
                                            <ul class="commentlist padding-0px margin-0px list-unstyled text-grey-3">
                                        <li class="border-bottom-1 border-grey-1 margin-bottom-20px">
                                            <img height="70px" width="70px" src="imgs/<?=isset($index['img_path'])? "{$index['img_path']}":"profile.png"?>" class="float-left margin-right-20px border-radius-60 margin-bottom-20px" alt="">
                                            <div class="margin-left-85px">
                                                <a class="d-inline-block text-dark text-medium margin-right-20px" href="#"><?=$index['first_name']." ".$index['last_name']?></a>
                                                <span class="text-extra-small">Date :  <a href="#" class="text-main-color"><?=$index['_datetime']?></a></span>
                                                <div class="rating">
